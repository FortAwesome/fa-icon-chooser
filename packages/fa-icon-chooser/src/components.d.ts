...
export namespace Components {
    ...
    interface FaIconChooser {
        ...
        "getUrlText": UrlTextFetcher;
        /**
          * Required callback function which is responsible for taking a given GraphQL query document and returns a Promise that resolves to a JavaScript object corresponding to the body of the associated network request, same as what would be produced by [Response.json()](https://developer.mozilla.org/en-US/docs/Web/API/Response/json).  The query document is compliant with the GraphQL API at [api.fontawesome.com](https://fontawesome.com/v5.15/how-to-use/graphql-api/intro/getting-started).  The implementation is responsible for handling any authorization that may be necessary to fulfill the request. For example, any time a kit is used to drive the Icon Chooser, it will be necessary to authorize GraphQL API requests sent to api.fontawesome.com with the [`kits_read` scope](https://fontawesome.com/v5.15/how-to-use/graphql-api/auth/scopes).  An optional third `options` argument may be supplied: `{ cache?: boolean, cacheKey?: string }`. In kit mode, the component requests caching of the kit metadata and of each family-style's page of showcase icons, by passing `{ cache: true, cacheKey }`. It first runs a small, `{ cache: false }` kit-identity probe (the kit's revision and showcase cache key); those values are folded into the `cacheKey` of every cacheable kit request. The `cacheKey` is a self-contained identity already derived from everything that affects the response (the query document, its variables, and the kit's revision and showcase cache key). An implementation that caches can therefore use `cacheKey` alone as its cache key, without re-deriving identity from the query or variables — and, because the probe that supplies the revision runs with `{ cache: false }`, a changed kit always yields a new `cacheKey`, so a long TTL on the cacheable requests is safe and will not pin the component to a stale kit. Honor `cache: false` (do not serve or store those responses from cache); implementations that ignore `options` entirely still work correctly (they simply cache less, or not at all).
         */
